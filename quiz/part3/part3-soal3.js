...
Jika stock barang kurang dari jumlah yang ingin dibeli oleh pembeli maka pembeli 
batal untuk membeli barang tersebut.

Function countProfit akan mengembalikan/me-return sebuah array of object 
dimana array tersebut berisi objek-objek barang dari toko X tersebut yang 
berisikan info : 

nama barang, 
siapa sajao yang membeli, 
sisa stock barang dan 
total pemasukan untuk barang tersebut
*/
...
function countProfit(shoppers) {
    ...
    for (let i = 0; i < listBarang.length; i++) {
      ...
      for (let j = 0; j < shoppers.length; j++) {
        if (shoppers[j].product === namaBarang && sisaStock >= shoppers[j].amount) {
          pembeli.push(shoppers[j].name);
          sisaStock -= shoppers[j].amount;
          totalProfit += shoppers[j].amount * harga;
        }
      }
